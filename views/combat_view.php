...
$currentHeroPv = $combatData['hero_pv'];
$maxHeroPv = $combatData['hero_max_pv'];
$currentMonsterPv = max(0, $combatData['monster_pv']);
$maxMonsterPv = $combatData['monster_max_pv'];
$isFinished = $combatData['finished'];
...
<head>
    ...
    <style>
        body, button { font-family: 'Press Start 2P', monospace; }
        .pixelated { image-rendering: pixelated; }
        /* Barre de vie simple CSS */
        .health-bar-bg { background-color: #5c4b43; height: 20px; width: 100%; border: 2px solid #2b2116; }
        .health-bar-fill { height: 100%; transition: width 0.3s; }
        .hp-high { background-color: #4ade80; }
        .hp-mid { background-color: #facc15; }
        .hp-low { background-color: #ef4444; }
    </style>
</head>
<body class="min-h-screen bg-gray-900 text-white p-4" style="background-image: url('<?php echo htmlspecialchars($chapter->getImage()); ?>'); background-size: cover; background-position: center;">
    
    <div class="max-w-4xl mx-auto bg-black/80 border-4 border-[#8b4513] p-6 rounded-lg shadow-2xl">
        
        <h1 class="text-center text-yellow-500 text-xl mb-6 uppercase tracking-widest border-b-2 border-yellow-500 pb-4">
        ...
        <div class="flex flex-col md:flex-row justify-between gap-8 mb-8">
            ...
            <div class="w-full md:w-1/2 bg-[#2a2a2a] p-4 rounded border-2 border-blue-500 relative">
                <h2 class="text-blue-300 mb-2"><?php echo htmlspecialchars($hero->getName()); ?> (<?php echo htmlspecialchars($hero->getClassName()); ?>)</h2>
                ...
                <div class="health-bar-bg mb-2">
                    ...
                    <div class="health-bar-fill <?php echo $pct > 50 ? 'hp-high' : ($pct > 20 ? 'hp-mid' : 'hp-low'); ?>" style="width: <?php echo $pct; ?>%;"></div>
                </div>

                <div class="mb-1 text-xs">Mana: <?php echo $combatData['hero_mana']; ?> / <?php echo $combatData['hero_max_mana']; ?></div>
                <div class="health-bar-bg border-blue-900 bg-gray-800">
                    <div class="health-bar-fill bg-blue-500" style="width: <?php echo ($combatData['hero_mana'] / $combatData['hero_max_mana']) * 100; ?>%;"></div>
                </div>
            ...
            <div class="w-full md:w-1/2 bg-[#2a2a2a] p-4 rounded border-2 border-red-500">
                ...
                <div class="flex justify-center mb-4">
                     <img src="<?php echo htmlspecialchars($monster->getImage()); ?>" class="h-32 object-contain pixelated drop-shadow-[0_0_10px_rgba(255,0,0,0.5)]">
                </div>

                <div class="mb-1 text-xs text-right">PV: <?php echo $currentMonsterPv; ?> / <?php echo $maxMonsterPv; ?></div>
                <div class="health-bar-bg mb-2">
                    <?php $m_pct = ($currentMonsterPv / $maxMonsterPv) * 100; ?>
                    <div class="health-bar-fill <?php echo $m_pct > 50 ? 'hp-high' : ($m_pct > 20 ? 'hp-mid' : 'hp-low'); ?>" style="width: <?php echo $m_pct; ?>%;"></div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-[#1a1a1a] p-4 rounded border-t-4 border-[#8b4513]">
            ...
            <?php if ($isFinished): ?>
                <div class="text-center py-6">
                    ...
                    <a href="index.php?action=continue&chapter=<?php echo $chapter->getId() + 1; ?>" class="bg-yellow-600 hover:bg-yellow-500 text-black px-6 py-3 rounded text-sm uppercase">
                        <?php echo $combatData['win'] ? 'Continuer l\'aventure' : 'Recommencer'; ?>
                    </a>
                </div>
            <?php else: ?>
                <form method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <button type="submit" name="action" value="physique" class="bg-gray-700 hover:bg-gray-600 border-b-4 border-gray-900 text-white py-4 px-2 rounded flex flex-col items-center group">
                        <span class="text-xl mb-2 group-hover:scale-110 transition">⚔️</span>
                        <span>Attaque Physique</span>
                    </button>

                    <?php if ($hero->isMage()): ?>
                        <button type="button" id="toggle-spells" class="bg-blue-900 hover:bg-blue-800 border-b-4 border-blue-950 text-white py-4 px-2 rounded flex flex-col items-center group">
                            <span class="text-xl mb-2 group-hover:scale-110 transition">✨</span>
                            <span>Magie</span>
                        </button>
                    <?php else: ?>
                        <button type="button" disabled class="bg-gray-800 text-gray-600 cursor-not-allowed border-b-4 border-gray-900 py-4 px-2 rounded flex flex-col items-center">
                    ...
                    <button type="submit" name="action" value="potion" class="bg-green-900 hover:bg-green-800 border-b-4 border-green-950 text-white py-4 px-2 rounded flex flex-col items-center group">
                        <span class="text-xl mb-2 group-hover:scale-110 transition">🧪</span>
                                ...
                                <div class="mb-3 p-2 bg-gray-900 rounded border border-gray-700">
                                    <div class="flex justify-between items-center">
                                        <div>
                                            ...
                                            <div class="text-xs text-gray-400"><?php echo htmlspecialchars($s->getDescription()); ?></div>
                                        </div>
                                    ...
                                    <div class="mt-2 text-right">
                                        <form method="POST" class="inline">
                                            ...
                                            <button type="submit" class="bg-blue-600 hover:bg-blue-500 text-white px-3 py-1 rounded text-xs" <?php echo ($combatData['hero_mana'] < $s->getManaCost()) ? 'disabled' : ''; ?>>
                                                Lancer
                                            </button>
                                        </form>
                                    </div>
